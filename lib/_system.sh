...
system_create_user() {
  ...
  sudo su - root <<EOF
  useradd -m -s /bin/bash -G sudo deploy 2>/dev/null || echo "Usuario ya existe, continuando..."
...
parar_firewall() {
  print_banner
  printf "${WHITE} 💻 Parando Firewall (atención su servidor quedará desprotegido)...${GRAY_LIGHT}"
  printf "\n\n"
...
system_unzip_whazing() {
  print_banner
  printf "${WHITE} 💻 Haciendo clone de whazing...${GRAY_LIGHT}"
  printf "\n\n"
...
apagar_distsrc() {
  print_banner
  printf "${WHITE} 💻 Borrando archivos de la versión anterior${GRAY_LIGHT}"
  printf "\n\n"
...
system_pm2_stop() {
  print_banner
  printf "${WHITE} 💻 Parando whazing...${GRAY_LIGHT}"
  printf "\n\n"
...
system_sysctl() {
  print_banner
  printf "${WHITE} 💻 Cambiando parámetros del sistema...${GRAY_LIGHT}"
  printf "\n\n"
...
system_docker_restart() {
  print_banner
  printf "${WHITE} 💻 Reiniciando contenedor docker...${GRAY_LIGHT}"
  printf "\n\n"
...
update_beta() {
  print_banner
  printf "${WHITE} 💻 Actualizando whazing Beta...${GRAY_LIGHT}"
  printf "\n\n"
